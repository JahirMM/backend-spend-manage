...
import { parseExpenseCount } from "@/lib/currency";

const URL_SERVICE = "/api/expenses/by-date";
const SERVICE_NAME = "expenses-by-date";

// POST /api/expenses/by-date
// Obtiene todos los gastos de un usuario en un mes completo (user_id, month, year en el body)
  ...
  try {
    ...
    try {
      expenses = await selectRows<Expense[]>(
        "expenses",
        `select=id,commerce,count,name,account_id,created_at&user_id=eq.${user_id}&created_at=gte.${dateStart}&created_at=lte.${dateEnd}&order=created_at.desc`,
      );
    } catch {
      await insertLog({
        url_service: URL_SERVICE,
    ...
    // Parsear count de formato moneda a número antes de retornar
    const parsed = parseExpenseCount(expenses);
